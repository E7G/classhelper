...
/** Account/password login, read-only course-resource sync, and authorized material browsing. */
class ChaoxingActivity : AppCompatActivity() {
    ...
    private lateinit var resourcesButton: MaterialButton
    ...
    private fun wireActions() {
        loginButton.setOnClickListener {
            ...
            val password = passwordEdit.text?.toString().orEmpty().ifBlank { s.chaoxingPassword }
            lifecycleScope.launch {
        ...
        resourcesButton.setOnClickListener {
            ...
            startActivity(ChaoxingMaterialsActivity.intentFor(this, course))
        }
        ...
        syncButton.setOnClickListener {
            ...
            lifecycleScope.launch {
                ...
                runCatching {
                    ...
                    setBusy(false, "同步完成 · ${result.chapters} 章 · ${result.resources} 个资源 · ${result.indexedChunks} 个知识片段 · 提取 ${result.extractedPdfPages} 页 PDF$skipped\n课堂问答会优先调用这门课程资源。")
                    renderSavedState()
                }.onFailure {
    ...
    private fun selectedCourse(): ChaoxingCourse? = courses.getOrNull(courseSpinner.selectedItemPosition)
    ...
    private suspend fun loadCoursesInternal() {
        runCatching { client.listCourses() }
            .onSuccess { loaded ->
                courses = loaded
                renderCourseSpinner()
                setBusy(false, "已读取 ${loaded.size} 门课程 · 可以查看资料或同步到知识库")
            }
    ...
    private fun renderSavedState() {
        ...
        if (s.chaoxingCourseName.isNotBlank()) {
            ...
            statusText.text = "当前课程资源：${sanitizeCourseName(s.chaoxingCourseName)}" + if (syncTime.isNotBlank()) "\n上次同步 $syncTime" else ""
        }
    }
    ...
    private fun renderCourseSpinner() {
        ...
        syncButton.isEnabled = hasCourses
        resourcesButton.isEnabled = hasCourses
    ...
    private fun renderCourseMeta(course: ChaoxingCourse?) {
        courseMetaText.text = when {
            ...
            course.teacher.isNotBlank() -> "教师：${course.teacher} · 可直接浏览本课程资料"
            else -> "可查看章节附件，也可同步章节和 PDF 文本到课堂知识库。"
        }
    }
    ...
    private fun setBusy(busy: Boolean, message: String) {
        ...
        resourcesButton.isEnabled = !busy && courses.isNotEmpty()
    ...
    private fun buildContent(): View {
        ...
        content.addView(TextView(this).apply {
            text = "登录自己的账号后，可以直接浏览课程附件；PDF 会交给课堂助手现有阅读器打开。只读取当前账号正常可访问的资源。"
            textSize = 13f
            setLineSpacing(0f, 1.18f)
            setPadding(0, dp(8), 0, dp(4))
        })

        val loginCard = sectionCard("账号登录", "账号会记住；密码和 Cookie 使用 AndroidKeyStore 加密保存。")
        ...
        val courseCard = sectionCard("选择课程", "这里只显示课程名称；课程 ID 和班级 ID 仅在内部访问资源。")
        ...
        resourcesButton = primaryButton("查看课程资料")
        courseCard.second.addView(resourcesButton, matchParams(top = 10, height = 56))
        syncButton = outlinedButton("同步到课堂知识库")
        courseCard.second.addView(syncButton, matchParams(top = 10, height = 52))

        val statusCard = sectionCard("状态", "资料查看不会提交学习进度或完成任务。")
